...
int8_t * PS2Mouse::report(int8_t data[]) {
  ...
  data[3] = read_movement_z(data[0]); // Z Movement Packet
  return data;
}
...
int8_t PS2Mouse::read_byte() {
  ...
  for (int i = 0; i < 8; i++) {
	int x_bit = read_bit();
    bitWrite(data, i, x_bit);
	sum_bits += x_bit;
  }
  int par_bit = read_bit(); // Partiy Bit
  int stop_bit = read_bit(); // Stop bit should be 1
  pull_low(_clock_pin);
  if (stop_bit != 1 || sum_bits%2 == par_bit) data = 0;
  return data;
}
...
int8_t PS2Mouse::read_movement_x(int status) {
  int8_t x = read();
  return x;
}

int8_t PS2Mouse::read_movement_y(int status) {
  int8_t y = read();
  return y;
}

int8_t PS2Mouse::read_movement_z(int status) {
	int8_t z = read();
	return z;
}
